...
function trimUnprocessableOptions(options: AppOptions): void {
    if (options.packager.platform === 'win32' && !isWindows() && !hasWine()) {
        const optionsPresent = Object.entries(options)
            .filter(
                ([key, value]) =>
                    OPTIONS_REQUIRING_WINDOWS_FOR_WINDOWS_BUILD.includes(key) &&
                    !!value,
            )
            .map(([key]) => key);
        ...
        for (const keyToUnset of optionsPresent) {
            (options as unknown as Record<string, undefined>)[keyToUnset] =
                undefined;
        }
    }
}
...
export async function buildNativefierApp(
    ...
    log.warn(
        '\n\n    Hi! Nativefier is abandoned by the original dude.\n' +
            "    There's now a hard fork at https://github.com/majick/nativefier\n" +
            "    Pull requests are accepted. Issues are not. I don't have time for that shit.\n\n",
    );
